...
export const Facultyleaderboard = () => {
  ...
  const fetchLeaderboard = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axios.get(`${API_BASE_URL}/leaderboard/faculty`);
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8">
      <h1 className="text-3xl font-bold mb-6">Faculty Leaderboard</h1>
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="space-y-4">
          {leaderboardData.map((player) => (
            <div
              key={player.id} // Ensure your API provides unique IDs for players
